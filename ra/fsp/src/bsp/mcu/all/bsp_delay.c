 ...
 * @param[in]   delay  The number of 'units' to delay_ms.
 ...
 *              A delay_ms of 2 us therefore requires 2000ns/332ns or 6 loops.
 *
 *              The 'theoretical' maximum delay_ms that may be obtained is determined by a full 32 bit loop count and the system clock rate.
 *              @120MHz:  ((0xFFFFFFFF loops * 4 cycles /loop) / 120000000) = 143 seconds.
 *              @32MHz:  ((0xFFFFFFFF loops * 4 cycles /loop) / 32000000) = 536 seconds
 *
 *              Note that requests for very large delays will be affected by rounding in the calculations and the actual delay_ms
 ...
 *              at all. In this case the requested delay_ms is too small (nanoseconds) to be carried out by the loop itself, and the
 *              overhead associated with executing the code to just get to this point has certainly satisfied the requested delay_ms.
 *
 ...
 *
 * @note This function will delay_ms for **at least** the specified duration. Due to overhead in calculating the correct number
 *       of loops to delay, very small delay_ms values (generally 1-5 microseconds) may be significantly longer than specified.
 ...
 *       to convert a calculated delay cycle count to a number of software delay_ms loops.
 *
 * @note Delays may be longer than expected when compiler optimization is turned off.
 *
 * @warning The delay_ms will be longer than specified on CM23 devices when the core clock is greater than 32 MHz. Setting
...
void R_BSP_SoftwareDelay (uint32_t delay, bsp_delay_units_t units)
{
    ...
    /** Only delay if the supplied parameters constitute a delay_ms. */
